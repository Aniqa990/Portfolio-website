...
(function(html) {
    ...
    * -------------------------------------------------- */
    ...
    const articles = document.querySelectorAll('.loading');
    const leftNav = document.querySelector('.backward');
    const rightNav = document.querySelector('.forward');
    ...
    function moveLeft() {
        if (currentIndex > 0) {
            ...
            currentIndex = articles.length - 1; // Loop to the end
        }
        reel.style.transform = `translateX(-${slideWidth * currentIndex}px)`;
    }
    ...
    leftNav.addEventListener('mouseover', () => {
        if (!isMoving) {
            isMoving = true;
            moveLeftInterval = setInterval(moveLeft, 1000);
        }
    });
    
    rightNav.addEventListener('mouseover', () => {
        if (!isMoving) {
            isMoving = true;
            moveRightInterval = setInterval(moveRight, 1000);
        }
    });
    
    leftNav.addEventListener('mouseout', () => {
